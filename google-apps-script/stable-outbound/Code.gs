...
function runtimeCanary_(b){
  ...
  const propsOk=p.getProperty('SB_PROVISIONED')==='1'&&p.getProperty('SB_ENV')==='STABLE'&&p.getProperty('SB_KIND')==='OUTBOUND'&&p.getProperty('SB_SHEET_ID')===ss.getId();if(!propsOk)return {ok:false,error:'STABLE_CANARY_PROPERTIES_MISMATCH'};
  const lock=LockService.getScriptLock();lock.waitLock(10000);
  try{
    const name='__STABLE_RUNTIME_CANARY';let sh=ss.getSheetByName(name);
    if(op==='CLEANUP'){
      if(!sh)return {ok:true,idempotent:true,environment_id:'STABLE',kind:'OUTBOUND',cleanup:true,properties_ok:true,bound_sheet:true};
      ...
      return {ok:true,idempotent:removed===0,environment_id:'STABLE',kind:'OUTBOUND',cleanup:true,removed:removed,properties_ok:true,bound_sheet:true};
    }
    ...
    if(hit)return {ok:true,idempotent:true,environment_id:'STABLE',kind:'OUTBOUND',row:hit.getRow(),properties_ok:true,bound_sheet:true};
    sh.appendRow([id,'STABLE','OUTBOUND',new Date().toISOString()]);SpreadsheetApp.flush();
    return {ok:true,idempotent:false,environment_id:'STABLE',kind:'OUTBOUND',row:sh.getLastRow(),properties_ok:true,bound_sheet:true};
