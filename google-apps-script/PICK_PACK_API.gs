...
function ppStableRuntimeCanary_(body){
  ...
  const token=String(body.google_access_token||''),id=String(body.canary_id||''),op=String(body.operation||'').toUpperCase(),ss=ppSs_();
  ...
  if(!token||!ppStableOwnerFile_(token,ss.getId(),'application/vnd.google-apps.spreadsheet'))return {ok:false,error:'STABLE_CANARY_OWNER_PROOF_FAILED'};
  ...
  try{
    ...
    if(op==='CLEANUP'){
      if(!sh)return {ok:true,idempotent:true,environment_id:'STABLE',kind:'PRIMARY',cleanup:true,properties_ok:true,bound_sheet:true};
      ...
      return {ok:true,idempotent:removed===0,environment_id:'STABLE',kind:'PRIMARY',cleanup:true,removed:removed,properties_ok:true,bound_sheet:true};
    }
    ...
    if(hit)return {ok:true,idempotent:true,environment_id:'STABLE',kind:'PRIMARY',row:hit.getRow(),properties_ok:true,bound_sheet:true};
    sh.appendRow([id,'STABLE','PRIMARY',ppNowIso_()]);SpreadsheetApp.flush();
    return {ok:true,idempotent:false,environment_id:'STABLE',kind:'PRIMARY',row:sh.getLastRow(),properties_ok:true,bound_sheet:true};
