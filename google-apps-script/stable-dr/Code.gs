...
function runtimeCanary_(b){
  ...
  const propsOk=p.getProperty('DR_PROVISIONED')==='1'&&p.getProperty('DR_ENV')==='STABLE'&&p.getProperty('DR_SHEET_ID')===ss.getId();
  ...
  try{
    ...
    if(op==='CLEANUP'){
      if(!sh)return {ok:true,idempotent:true,operation:'CLEANUP',environment_id:'STABLE',kind:'DR',cleanup:true,properties_ok:true,bound_sheet:true};
      ...
      return {ok:true,idempotent:removed===0,operation:'CLEANUP',environment_id:'STABLE',kind:'DR',removed:removed,cleanup:true,properties_ok:true,bound_sheet:true};
    }
    ...
    if(hit)return {ok:true,idempotent:true,operation:'UPSERT',environment_id:'STABLE',kind:'DR',row:hit.getRow(),properties_ok:true,bound_sheet:true};
    sh.appendRow([id,'STABLE','DR',new Date().toISOString()]);SpreadsheetApp.flush();
    return {ok:true,idempotent:false,operation:'UPSERT',environment_id:'STABLE',kind:'DR',row:sh.getLastRow(),properties_ok:true,bound_sheet:true};
