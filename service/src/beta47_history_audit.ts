...
async function existingIds(env:Env,t:string):Promise<Set<string>>{if(isStableEnvironment(env)){const j=await stableSheetBridge<{ok:true;values?:unknown[][]}>(env,"primary","get_values",{sheet:"LỊCH SỬ NGHIỆP VỤ",range:"K2:K"});return new Set((j.values??[]).map(x=>text(x[0])).filter(Boolean));}const range=`${q("LỊCH SỬ NGHIỆP VỤ")}!K2:K`,r=await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(env.GOOGLE_SOURCE_SHEET_ID)}/values/${encodeURIComponent(range)}?majorDimension=ROWS&valueRenderOption=FORMATTED_VALUE`,{headers:{authorization:`Bearer ${t}`}});if(!r.ok)throw new Error(`BETA47_HISTORY_READ:${r.status}`);const rows=(await r.json<{values?:unknown[][]}>()).values??[];return new Set(rows.map(x=>text(x[0])).filter(Boolean));}
async function append(env:Env,t:string,rows:unknown[][]):Promise<void>{if(!rows.length)return;if(isStableEnvironment(env)){await stableSheetBridge(env,"primary","append_values",{sheet:"LỊCH SỬ NGHIỆP VỤ",range:"A:M",values:rows});return;}const range=`${q("LỊCH SỬ NGHIỆP VỤ")}!A:M`,r=await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(env.GOOGLE_SOURCE_SHEET_ID)}/values/${encodeURIComponent(range)}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`,{method:"POST",headers:{authorization:`Bearer ${t}`,"content-type":"application/json"},body:JSON.stringify({range,majorDimension:"ROWS",values:rows})});if(!r.ok){const x=await r.text();throw new Error(`BETA47_HISTORY_APPEND:${r.status}:${x.slice(0,220)}`);}}
