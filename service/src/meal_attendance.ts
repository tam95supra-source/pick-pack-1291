...
function timeMs(v:string):number{return Date.parse(v)||0;}
function nowBangkokParts(){const now=new Date(Date.now()+7*3600_000);return{date:now.toISOString().slice(0,10),minutes:now.getUTCHours()*60+now.getUTCMinutes()};}
...
function statusView(row:Record<string,unknown>,now:number):string{
  ...
  if(status==="LATE_EXPECTED"){
    const expected=timeMs(String(row.expected_return_at||""));
    if(expected>0&&now>=expected&&!row.actual_return_at)return"OVERDUE_LATE";
  }
  return status;
}
...
export async function mealAttendanceList(env:Env,body:{business_date?:string}):Promise<Response>{
  ...
  const now=Date.now();
  const items=rows.map(r=>({...r,status_view:statusView(r,now)})).sort((a,b)=>{
