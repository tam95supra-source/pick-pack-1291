#!/usr/bin/env bash
set -Eeuo pipefail
R=ops/beta-release-request.json;OUT=/tmp/beta84-pda;rm -rf "$OUT";mkdir -p "$OUT"
...
NEW=/tmp/beta84-candidate/pick-pack-1291-public-beta-$VERSION.apk
PUB=/tmp/beta84-publish/receipt.json;test -f "$OLD" -a -f "$NEW" -a -f "$PUB" -a -f "$VERIFY_HARNESS_APK"
