...
grep -Fq 'val mealWarning=PostMealAttendanceFeature.buildHomeWarning(this,api){postMealAttendanceScreen()}' "$OPS"
...
grep -Fq 'root.addView(DropReceiveFeature.build(this,api,login,name,role,{label,after->verifyActionPassword(label,after)}){businessHome()},LinearLayout.LayoutParams(-1,0,1f))' "$OPS"
...
for spec in '320 568 160' '360 640 180' '480 800 240'; do
  ...
  if [[ "$MODE" == checks ]]; then
    ...
    for flag in current_day_filter header_removed header_sync_chip attendance_card root_back_stays old_warning_preserved qr_reconciliation null_sanitized shift_quick_exit_dialog_beta113 post_scan_roster_hidden_beta124 post_scan_activity_back_beta124 inline_shift_staff_beta113 detail_reconciliation_visible shift_staff_grouped_ncc_beta105 shift_staff_filter_counts_beta105 staff_list_to_qr settings_download_qr_beta105 complete_quick_summary_beta113 staff_contact_layout staff_search_fixed settings_simplified dual_changelog authoritative_editor_offline_guard session_exit_identity_guard pda_exit_only_when_session_has_pda old_warning_unified report_columns_aligned log_metadata_persisted reconciliation_above_scan work_info_order qr_employee_contact pick_phone_account owner_actions_above_shift owner_100_sessions_projection_beta118 before_after_visible timeline_changed_fields_only employee_timeline_realtime_functional assignment_snapshot_authoritative scalar_snapshot_fallback timeline_newest_first hhmm_edit_confirmation delete_reason_editable report_compact_grid report_available_dates_only meal_attendance_module meal_current_day_scan meal_duplicate_local meal_invalid_employee_guard status_header_meal meal_old_session_blocked history_hidden_user history_deeplink_blocked_user qr_local_not_entered_no_service_wait status_chip_details_beta100 document_management_card_beta107 document_management_controls_beta107 document_pending_ui_beta108 document_pending_durable_beta108 document_selected_draft_durable_beta109 document_selected_multi_draft_beta110 document_batch_controls_beta110 document_category_cache_offline_beta109 document_media_cache_beta108 document_category_rename_delete_owner_rule_beta108 labor_open_list_beta110 navigation_history_beta111 review_warning_shared_style_beta112 meal_null_dash_beta110 resilience_scenario_selectable resilience_test_ledger_result resilience_business_outbox_isolated resilience_options_bordered_beta101 resilience_history_cards_beta101 resilience_stop_control_beta101; do
...
expected={"320x568":((320,568),11),"360x640":((360,640),11),"480x800":((480,800),11)} if visual_only else {"320x568":((320,568),21),"360x640":((360,640),11),"480x800":((480,800),11)}
