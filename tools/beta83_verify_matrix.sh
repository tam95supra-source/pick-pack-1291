...
for spec in '320 568 160' '360 640 180' '480 800 240'; do
  ...
  if [[ "$MODE" == checks ]]; then
    ...
    for flag in current_day_filter header_removed header_sync_chip attendance_card root_back_stays old_warning_preserved qr_reconciliation null_sanitized incomplete_detail_button detail_reconciliation_visible staff_list_to_qr complete_direct_list staff_contact_layout staff_search_fixed settings_simplified log_metadata_persisted reconciliation_above_scan work_info_order qr_employee_contact pick_phone_account owner_actions_above_shift before_after_visible timeline_changed_fields_only assignment_snapshot_authoritative scalar_snapshot_fallback timeline_newest_first hhmm_edit_confirmation delete_reason_editable report_compact_grid report_available_dates_only; do
