...
jq -e --arg e "$R5_ENTER" '.ok==true and .results[0].local_event_id==$e and .results[0].status=="CONFIRMED" and .results[0].canonical_event_id==$e' "$D/r5-ws-fixture-enter.json" >/dev/null
...
R5_WS_OUT="$D/r5-live-measurement" node tools/r5_beta130_ws_convergence.mjs
...
'''
new2='''jq --argjson before "$SHEETS_USED_BEFORE" --argjson after "$SHEETS_USED_AFTER" --arg source_sha "${SERVICE_SOURCE_SHA:-}" --arg run_id "${GITHUB_RUN_ID:-}" '. + {sheets_quota:{daily_before:$before,daily_after:$after,delta:($after-$before),google_api_calls_from_gate:0},exact_service_source_sha:$source_sha,github_run_id:$run_id}' "$D/r5-live-measurement/receipt.json" > "$D/receipt.json"
jq -e '.status=="PASS" and .classification=="EXACT_DEPLOYED_SERVICE_WS_WAKE_DELTA_5_ISOLATED_AUTH_SESSIONS" and .auth_sessions.isolated==true and .clients.total==5 and .clients.android_pda==3 and .clients.web==2 and .samples==50 and .sheets_quota.delta==0 and .remote_convergence_ms.p95<=1000 and .remote_convergence_ms.p99<=2000' "$D/receipt.json" >/dev/null
'''
