#!/usr/bin/env bash
set -Eeuo pipefail
R=ops/beta-release-request.json;OUT=/tmp/beta83-pda;rm -rf "$OUT";mkdir -p "$OUT"
...
OLD=/tmp/beta82/pick-pack-1291-public-beta-0.4.2-beta.82.apk
NEW=/tmp/beta83-candidate/pick-pack-1291-public-beta-$VERSION.apk
PUB=/tmp/beta83-publish/receipt.json;test -f "$OLD" -a -f "$NEW" -a -f "$PUB" -a -f "$VERIFY_HARNESS_APK"
...
adb shell dumpsys package "$PKG" > "$OUT/pkg81.txt";grep -Fq 'versionName=0.4.2-beta.82' "$OUT/pkg81.txt";grep -Eq 'versionCode=88([[:space:]]|$)' "$OUT/pkg81.txt"
...
update BETA 0.4.2-beta.82 "$OUT/beta-old.json";jq -e --arg v "$VERSION" --arg h "$SHA" --argjson z "$SIZE" '.ok==true and .available==true and .version_name==$v and .sha256==$h and .size==$z' "$OUT/beta-old.json" >/dev/null
