...
jq -n --arg v "$VERSION" --argjson c "$CODE" --arg base "$BASE" --arg h "$SHA" --argjson z "$SIZE" --arg signer "$INST_SIGNER" --arg main "$MAIN_AFTER"   --slurpfile beta "$OUT/beta-old.json" --slurpfile current "$OUT/beta-current.json" --slurpfile stable "$OUT/stable.json" --slurpfile auth "$OUT/authority.json"   '{status:"PASS",version_name:$v,version_code:$c,base_version:$base,apk_sha256:$h,apk_size:$z,signer_sha256:$signer,ota_from_base:true,ota_download_exact:true,
