#!/usr/bin/env bash
set -Eeuo pipefail
R=ops/beta-release-request.json;E=/tmp/beta85-rollback;rm -rf "$E";mkdir -p "$E";PUB=/tmp/beta85-publish/receipt.json;test -f "$PUB"
...
test "$PASS" = 1
jq -n --slurpfile r "$E/readback.json" '{status:"PASS",rollback_beta83:true,beta_readback:$r[0]}' > "$E/receipt.json";cat "$E/receipt.json"
