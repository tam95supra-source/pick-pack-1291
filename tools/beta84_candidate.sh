...
jq -e '.stage=="BUILD_VERIFY" and .version_name=="0.4.2-beta.84" and .version_code==90 and .stable_publish=="FORBIDDEN" and .authority_change=="NONE" and .human_visual_pass==false' "$REQ" >/dev/null
...
test -f "$U";"$BT/aapt" dump badging "$U" > /tmp/beta84-badging.txt
grep -q "package: name='$PKG'" /tmp/beta84-badging.txt
grep -q "versionCode='$CODE'" /tmp/beta84-badging.txt
grep -q "versionName='$VERSION'" /tmp/beta84-badging.txt
...
OUT=/tmp/beta84-candidate;rm -rf "$OUT";mkdir -p "$OUT"
