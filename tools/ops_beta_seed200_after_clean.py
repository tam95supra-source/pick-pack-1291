...
TARGET={'Ca 1':80,'Ca 2':80,'Ca HC':40};PREFIX='OWNER_SEED200_20260904_'
...
def must(url,method='GET',token=None,body=None,timeout=60):
 ...
 if c//100!=2 or j.get('ok') is not True:raise RuntimeError('SERVICE:'+str(c)+':'+str(j.get('error') or j)[:700])
 return j
def rec(status,**x):
...
def cleanup():
 ...
 if LOGIN:
  ...
  LOGIN=''
def sameval(a,b):return ('' if a is None else str(a))==('' if b is None else str(b))
try:
 ...
 for shift,n in TARGET.items():
  specialist=take('SPECIALIST',1);leader=take('LEADER',1);pull=take('PULL',3);pick=take('PICK',1);pack=take('PACK',1);crew=specialist+leader+pull+pick+pack
  ...
  for x in crew+fill:
   work='PICK' if x['mnv']==pick[0]['mnv'] else ('PACK' if x['mnv']==pack[0]['mnv'] else 'KHONG');plan.append(dict(x,shift=shift,work_choice=work))
 ...
 used_tables=set();used_pack_users=set();resource_used={'PDA':[],'USER_PICK':[],'PACK_PAIR':[]}
 for shift in TARGET:
  ...
  pick_emp['pda_serial']=pdas.pop(0);pick_emp['user_pick']=picks.pop(0)
  pair=next((x for x in by[shift] if x['pack_table'] not in used_tables and x['user_pack'] not in used_pack_users),None)
  if pair is None:raise RuntimeError('PACK_RESOURCE_UNIQUE_CAPACITY:'+shift)
  used_tables.add(pair['pack_table']);used_pack_users.add(pair['user_pack']);pack_emp['pack_table']=pair['pack_table'];pack_emp['user_pack']=pair['user_pack']
 ...
 for i,x in enumerate(plan,1):
  ...
  for k in ('pda_serial','user_pick','pack_table','user_pack'):
   if x.get(k):p[k]=x[k]
  ev.append({'action':'enter','event_id':f'{PREFIX}{i:03d}_{x["mnv"]}','device_id':'OWNER_SEED200','business_date':TODAY,'payload':p})
 expected_ids={x['event_id'] for x in ev};expected_by_mnv={x['payload']['mnv']:x['payload'] for x in ev}
 current=d1('SELECT session_id,mnv,shift,work_choice,state,pda_serial,user_pick,pack_table,user_pack,exit_at FROM attendance_sessions WHERE business_date='+q(TODAY))
 labor_n=int(d1('SELECT COUNT(*) n FROM labor_sessions WHERE business_date='+q(TODAY))[0]['n']);meal_n=int(d1('SELECT COUNT(*) n FROM post_meal_attendance WHERE business_date='+q(TODAY))[0]['n'])
 owner_events=d1("SELECT event_id,entity_id FROM events WHERE business_date="+q(TODAY)+" AND event_id LIKE 'OWNER_SEED200_20260904_%'")
 owner_ids={x['event_id'] for x in owner_events};owner_entities={x['entity_id'] for x in owner_events}
 if labor_n!=0:raise RuntimeError('RESUME_LABOR_NOT_ZERO')
 if len(current)>200 or len(owner_events)!=len(current) or meal_n!=len(current):raise RuntimeError('RESUME_COUNT_GUARD')
 if any(x['session_id'] not in owner_entities for x in current):raise RuntimeError('RESUME_NON_OWNER_ATTENDANCE')
 if not owner_ids.issubset(expected_ids):raise RuntimeError('RESUME_EVENT_SET_DRIFT')
 for r in current:
  p=expected_by_mnv.get(r['mnv'])
  if p is None or r['state']!='ACTIVE' or r.get('exit_at') not in (None,'') or r['shift']!=p['shift'] or r['work_choice']!=p['work_choice']:raise RuntimeError('RESUME_ATTENDANCE_DRIFT:'+str(r['mnv']))
  for k in ('pda_serial','user_pick','pack_table','user_pack'):
   if not sameval(r.get(k),p.get(k)):raise RuntimeError('RESUME_RESOURCE_DRIFT:'+str(r['mnv'])+':'+k)
 ...
 for i in range(0,200,25):
  ...
  if len(rs)!=len(ev[i:i+25]) or bad:
   rec('FAIL',error='BATCH_REJECT',batch_start=i+1,batch_end=i+25,result_count=len(rs),bad_results=bad[:10],result_sample=rs[:25],resume_from=resume_from,stable_unchanged=True);raise RuntimeError('BATCH_REJECT_DETAILS_RECORDED')
 ...
 meal_final=int(d1('SELECT COUNT(*) n FROM post_meal_attendance WHERE business_date='+q(TODAY))[0]['n'])
 if meal_final!=200:raise RuntimeError('MEAL_PROJECTION_NOT_200')
 cleanup();rec('PASS',resume_from=resume_from,confirmed=confirmed,duplicates=duplicates,active=200,post_meal_projection=meal_final,shift_distribution=shifts,work_choice_distribution=works,core_composition=core,resource_used=resource_used,d1_readback='PASS',stable_unchanged=True);print(json.dumps({'status':'PASS','resume_from':resume_from,'confirmed':confirmed,'duplicates':duplicates,'active':200,'shift':shifts,'work':works,'core':core,'resources':resource_used},ensure_ascii=False))
except Exception as e:
 cleanup()
 if not OUT.exists() or json.loads(OUT.read_text()).get('status')!='FAIL':rec('FAIL',error=str(e)[:1200],stable_unchanged=True)
 print('SEED200_ERROR:'+str(e),file=sys.stderr);sys.exit(1)
