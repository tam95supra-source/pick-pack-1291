...
ROLL=$(jq -r '.jobs[]?|select(.name=="rollback-beta82")|.conclusion' <<<"$JOBS"|head -n1)
if [[ "$FIN" == success ]]; then NEXT=WAIT_FOR_OWNER_NEW_SCOPE
elif [[ "$ROLL" == success ]]; then NEXT=FIX_POST_PUBLISH_BETA83_FAILURE_WITH_BETA82_RESTORED
elif [[ "$PUB" != success ]]; then NEXT=FIX_BETA83_PUBLISH_FAILURE_KEEP_EXACT_LOCKED_CANDIDATE
else NEXT=FIX_BETA83_POST_PUBLISH_OTA_VERIFY_KEEP_EXACT_LOCKED_CANDIDATE
fi
{
...
echo 'PICK PACK 1291 — AUTOMATED BETA83 RELEASE HANDOFF'
echo 'status=READY'
