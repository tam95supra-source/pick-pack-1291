...
if [[ "$CAND" != success ]]; then NEXT=FIX_BETA85_CANDIDATE_FAILURE
elif [[ "$VERIFY" != success ]]; then NEXT=FIX_BETA85_VERIFY_FAILURE_KEEP_LOCKED_CANDIDATE_IF_BYTES_VALID
else NEXT=HUMAN_INSPECT_BETA85_VISUALS_THEN_PUBLISH_EXACT_LOCKED_CANDIDATE_IF_PASS
fi
{
...
echo 'PICK PACK 1291 — AUTOMATED BETA85 PRE-OTA HANDOFF'
echo 'status=READY'
