...
APK=/tmp/beta85-candidate/pick-pack-1291-public-beta-$VERSION.apk
META=/tmp/beta85-candidate/release-meta.json
VERIFY=/tmp/beta85-verify/receipt.json
...
jq -nc --arg d "Beta85 exact OTA; SHA256 $SHA; candidate run $(jq -r '.candidate_run_id' "$R"); artifact $(jq -r '.candidate_artifact_id' "$R"); signer $SIGNER" '{description:$d}' > "$E/meta.json"
