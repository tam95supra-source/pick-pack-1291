...
jq -e '.stage=="BUILD_VERIFY" and .version_name=="0.4.2-beta.85" and .version_code==91 and .stable_publish=="FORBIDDEN" and .authority_change=="NONE" and .human_visual_pass==false' "$REQ" >/dev/null
...
grep -Fq 'validActionTimePassword' "$OPS"
grep -Fq 'DateTimeFormatter.ofPattern("HHmm")' "$OPS"
grep -Fq 'Regex("""\d{4}""")' "$OPS"
! grep -Fq 'setMessage("Nhập mật khẩu HH:mm' "$OPS"
grep -Fq 'private fun shiftStaffOrdered' "$OPS"
grep -Fq 'merge(parsed(e.opt("payload_json")))' "$OPS"
grep -Fq 'out.sortByDescending{atMillis(it)}' "$OPS"
grep -Fq 'pruneManagedUpdateApks' app/src/main/java/vn/pickpack1291/app/beta/UpdateManager.kt
...
test -f "$U";"$BT/aapt" dump badging "$U" > /tmp/beta85-badging.txt
grep -q "package: name='$PKG'" /tmp/beta85-badging.txt
grep -q "versionCode='$CODE'" /tmp/beta85-badging.txt
grep -q "versionName='$VERSION'" /tmp/beta85-badging.txt
...
OUT=/tmp/beta85-candidate;rm -rf "$OUT";mkdir -p "$OUT"
