...
elif [[ "$ROLL" == success ]]; then NEXT=FIX_POST_PUBLISH_BETA84_FAILURE_WITH_BETA83_RESTORED
elif [[ "$PUB" != success ]]; then NEXT=FIX_BETA84_PUBLISH_FAILURE_KEEP_EXACT_LOCKED_CANDIDATE
else NEXT=FIX_BETA84_POST_PUBLISH_OTA_VERIFY_KEEP_EXACT_LOCKED_CANDIDATE
fi
{
...
echo 'PICK PACK 1291 — AUTOMATED BETA84 RELEASE HANDOFF'
echo 'status=READY'
