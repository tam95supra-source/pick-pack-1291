...
def ensure_once(path:str,old:str,new:str,label:str):
    p=ROOT/path;s=p.read_text(encoding='utf-8')
    if new in s:
        if s.count(new)!=1:
            raise SystemExit(f'R5_BETA129_TARGET_NOT_UNIQUE:{label}:{s.count(new)}')
        return False
    if old not in s:
...
if new not in s:
    ...
    raise SystemExit(f'R5_BETA129_RELEASE_NOTES_TARGET_NOT_UNIQUE:{s.count(new)}')
...
b=Path(ROOT/'app/build.gradle.kts').read_text(encoding='utf-8')
assert 'versionCode = 135\n            versionName = "0.4.2-beta.129"' in b
assert 'Beta129: R5 quota/realtime architecture' in b
assert new in notes.read_text(encoding='utf-8')
assert 'cp tools/R5PerfInstrumentation.java' in build.read_text(encoding='utf-8')
assert '.R5PerfInstrumentation' in build.read_text(encoding='utf-8')
assert 'R5_LOCAL_UI_P95_PASS' in matrix.read_text(encoding='utf-8')
