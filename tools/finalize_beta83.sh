...
PUB=/tmp/beta83-publish-final/receipt.json
PDA=/tmp/beta83-pda-final/receipt.json
test -f "$PUB" -a -f "$PDA"
...
jq -e --arg v "$VERSION" --arg h "$SHA" --argjson z "$SIZE" --arg s "$SIGNER" '
 ...
 .ota_from_beta82==true and .ota_download_exact==true and .installed_exact_bytes==true and .installed_and_opened==true and
...
ARCH="docs/handovers/HANDOVER_${STAMP}_beta83-pass-live.md"
...
git commit -m '[skip ci] finalize Beta83 PASS/LIVE'
FINAL=$(git rev-parse HEAD)
git push origin "HEAD:$BRANCH"
git fetch origin "$BRANCH" --quiet
test "$(git rev-parse "origin/$BRANCH")" = "$FINAL"
test "$(git show "origin/$BRANCH:CURRENT_STATE.md"|grep -c BETA83_PASS_LIVE)" = 1
test "$(git show "origin/$BRANCH:docs/handovers/HANDOVER_CURRENT.md"|grep -c 'status: READY')" = 1
mkdir -p /tmp/beta83-final
jq -n --arg status PASS --arg version "$VERSION" --arg source "$SOURCE" --arg sha "$SHA" --argjson size "$SIZE" --arg signer "$SIGNER" --arg commit "$FINAL" --arg main "$MAIN" --arg at "$NOW" \
 '{status:$status,version_name:$version,source_sha:$source,apk_sha256:$sha,apk_size:$size,signer_sha256:$signer,handoff_commit_sha:$commit,main_sha:$main,stable_unchanged:true,authority_change:"NONE",readback:true,finalized_at:$at}' > /tmp/beta83-final/receipt.json
cat /tmp/beta83-final/receipt.json
