...
W=/tmp/beta82-harness
rm -rf "$W";mkdir -p "$W/src/vn/pickpack1291/verify" "$W/classes" "$W/dex"
sed -e 's/Beta80 OTA exact candidate/Beta82 OTA exact candidate/g' -e 's/putInt("version_code",86)/putInt("version_code",88)/g' tools/Beta80VerifyInstrumentation.java > "$W/src/vn/pickpack1291/verify/Beta80VerifyInstrumentation.java"
cp tools/Beta82UiChecksInstrumentation.java "$W/src/vn/pickpack1291/verify/"
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android" package="vn.pickpack1291.verify">
  ...
  <application android:label="Beta82 Verify"/>
  ...
  <instrumentation android:name=".Beta82UiChecksInstrumentation" android:targetPackage="vn.pickpack1291.app.beta.publicbeta" android:functionalTest="true" android:handleProfiling="false"/>
</manifest>
XML
...
"$BT/apksigner" sign --ks "$KS" --ks-key-alias "$SIGNING_ALIAS" --ks-pass "file:$W/store.pass" --key-pass "file:$W/key.pass" --out /tmp/beta82-verify-harness.apk "$W/harness-unsigned.apk"
"$BT/apksigner" verify --print-certs /tmp/beta82-verify-harness.apk > "$W/cert.txt"
...
echo "VERIFY_HARNESS_APK=/tmp/beta82-verify-harness.apk" >> "$GITHUB_ENV"
