...
# Beta70 is exactly one beta after the locked/failed Beta69 candidate.
g = GRADLE.read_text()
g = replace_once(
    g,
    'versionCode = 75\n            versionName = "0.4.2-beta.69"',
    'versionCode = 76\n            versionName = "0.4.2-beta.70"',
    'Beta70 version',
)
GRADLE.write_text(g)
...
new = '''                listBox.removeAllViews();if(handleAuth(rr))return@runOnUiThread
                // Beta70: local active sessions are the first authority for the PDA holder list.
...
elif 'Beta70: local active sessions are the first authority for the PDA holder list.' not in ops:
    raise SystemExit('Beta70 PDA local-holder anchor missing')

OPS.write_text(ops)

# Static regression against the materialized Android source.
assert 'versionCode = 76' in g and 'versionName = "0.4.2-beta.70"' in g
assert 'versionCode = 1' in g and 'versionName = "0.1.0-stable"' in g
assert 'Beta70: local active sessions are the first authority for the PDA holder list.' in ops
...
print('BETA70_PDA_LOCAL_HOLDER_FIX_PASS')
