...
adb shell cat "/data/user/0/$PKG/shared_prefs/pp_m2_service_transport.xml" > "$OUT/cache-after.xml"
! grep -Fq '>https://pickpack1291.cc.cd<' "$OUT/cache-after.xml"
SERVICE_URL=$(sed -n 's/^INSTRUMENTATION_STATUS: service_url=//p' "$OUT/instrument.txt"|tail -n1|tr -d '\r')
ENV_ID=$(sed -n 's/^INSTRUMENTATION_STATUS: environment_id=//p' "$OUT/instrument.txt"|tail -n1|tr -d '\r')
AUD=$(sed -n 's/^INSTRUMENTATION_STATUS: service_audience=//p' "$OUT/instrument.txt"|tail -n1|tr -d '\r')
test "$ENV_ID" = "BETA"
test "$AUD" = "PICK_PACK_1291_BETA"
[[ "$SERVICE_URL" == https://* ]]
test "$SERVICE_URL" != "https://pickpack1291.cc.cd"
jq -n --arg v "$VERSION" --arg h "$SHA" --argjson z "$SIZE" --arg env "$ENV_ID" --arg aud "$AUD" --arg url "$SERVICE_URL"   '{status:"PASS",version_name:$v,apk_sha256:$h,apk_size:$z,service_discovery_cache_regression:"PASS",stale_cache_seeded:true,ttl_unexpired:true,environment_id:$env,service_audience:$aud,service_url:$url,stable_root_reused:false,app_data_cleared:false}' > "$OUT/receipt.json"
