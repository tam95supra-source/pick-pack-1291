...
PUB=/tmp/beta84-publish-final/receipt.json
PDA=/tmp/beta84-pda-final/receipt.json
test -f "$PUB" -a -f "$PDA"
...
ARCH="docs/handovers/HANDOVER_${STAMP}_beta84-pass-live.md"
...
## Mục tiêu + DoD
Beta84 sửa vị trí rà soát nhân sự, thứ tự thông tin công việc, chi tiết trước/sau cập nhật, timeline mới tới cũ và xác thực thao tác HH:mm; toàn bộ pre-OTA + OTA install/readback PASS.
...
- OTA Beta83 → Beta84 exact bytes, SHA/size/signer/version và mở app: PASS.
...
git commit -m '[skip ci] finalize Beta84 PASS/LIVE'
FINAL=$(git rev-parse HEAD)
git push origin "HEAD:$BRANCH"
git fetch origin "$BRANCH" --quiet
test "$(git rev-parse "origin/$BRANCH")" = "$FINAL"
test "$(git show "origin/$BRANCH:CURRENT_STATE.md"|grep -c BETA84_PASS_LIVE)" = 1
test "$(git show "origin/$BRANCH:docs/handovers/HANDOVER_CURRENT.md"|grep -c 'status: READY')" = 1
mkdir -p /tmp/beta84-final
jq -n --arg status PASS --arg version "$VERSION" --arg source "$SOURCE" --arg sha "$SHA" --argjson size "$SIZE" --arg signer "$SIGNER" --arg commit "$FINAL" --arg main "$MAIN" --arg at "$NOW" \
 '{status:$status,version_name:$version,source_sha:$source,apk_sha256:$sha,apk_size:$size,signer_sha256:$signer,handoff_commit_sha:$commit,main_sha:$main,stable_unchanged:true,authority_change:"NONE",readback:true,finalized_at:$at}' > /tmp/beta84-final/receipt.json
cat /tmp/beta84-final/receipt.json
