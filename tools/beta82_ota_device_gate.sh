#!/usr/bin/env bash
set -Eeuo pipefail
R=ops/beta-release-request.json;OUT=/tmp/beta82-pda;rm -rf "$OUT";mkdir -p "$OUT"
...
OLD=/tmp/beta81/pick-pack-1291-public-beta-0.4.2-beta.81.apk
NEW=/tmp/beta82-candidate/pick-pack-1291-public-beta-$VERSION.apk
PUB=/tmp/beta82-publish/receipt.json;test -f "$OLD" -a -f "$NEW" -a -f "$PUB" -a -f "$VERIFY_HARNESS_APK"
...
adb shell dumpsys package "$PKG" > "$OUT/pkg81.txt";grep -Fq 'versionName=0.4.2-beta.81' "$OUT/pkg81.txt";grep -Eq 'versionCode=87([[:space:]]|$)' "$OUT/pkg81.txt"
...
for _ in $(seq 1 180); do
  ...
  if grep -Fq "versionName=$VERSION" "$OUT/pkg-current.txt" && grep -Eq "versionCode=$CODE([[:space:]]|$)" "$OUT/pkg-current.txt"; then cp "$OUT/pkg-current.txt" "$OUT/pkg82.txt";UPDATED=1;break;fi
  sleep .5
done
