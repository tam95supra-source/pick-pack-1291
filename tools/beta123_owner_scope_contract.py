...
need={
 ...
 'settings':['confirmClearCache','confirmResetLocalAppData','clearApplicationUserData','XÓA CACHE','ĐẶT LẠI DỮ LIỆU'],
 'history':['Tìm mã nhân viên, họ tên, nghiệp vụ …','XÓA TOÀN BỘ','deleteHistoryBulk(all)'],
...
for x in need['beta123']:
 assert x in gradle,x
for x in need['notes']:
 assert x in notes,x
for k in ['settings','history','status','report','labor','qr']:
...
# Existing guards must remain.
for x in ['if(!isAdmin())','if(!isSuper())','renderEmployeeIfChanged','SUPERADMIN']:
 assert x in ops,x
