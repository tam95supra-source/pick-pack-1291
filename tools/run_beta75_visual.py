...
def preflight():
    src=Path(__file__).read_text(encoding='utf-8');compile(src,'run_beta75_visual.py','exec')
    assert "module','PDA_EXCHANGE'" in src
    assert "pp_operational_45d.db" in src
    assert "320x568" in src and "360x640" in src and "480x800" in src
    assert 'uiautomator' not in src and 'am instrument' not in src
    print(json.dumps({'py_compile':'PASS','matrix':[x[0] for x in MATRIX],'screen':'PDA_EXCHANGE','fixture':'local sqlite + master prefs','gate':'dumpsys activity/window + real PNG + human inspection'},ensure_ascii=False))
...
def make_db(path,date):
    ...
    sessions=[
      {'session_id':'visual-s1','mnv':'30011','business_date':date,'shift':'Ca 2','state':'ACTIVE','pda_serial':'NLS-MT90-0012345','pda_enter_status':'Nguyên vẹn','enter_at':f'{date}T10:10:00Z','version':4},
      {'session_id':'visual-s2','mnv':'30050','business_date':date,'shift':'Ca 2','state':'ACTIVE','pda_serial':'NLS-MT90-0067890','pda_enter_status':'Màn hình xước nhẹ','enter_at':f'{date}T10:15:00Z','version':2},
    ]
    snap={'ok':True,'business_date':date,'day_revision':7,'sessions':sessions,'events':[],'labor':[]}
...
def seed_fixture():
    adb('root',check=False);adb('wait-for-device');adb('shell','am','force-stop',PKG,check=False)
    ...
    auth=OUT/'pick_pack_auth_session_v2.xml';auth.write_text(prefs_xml({'token':'beta75-visual-offline-token','login_id':'0987654321','display_name':'Nguyễn Văn Tâm','role':'ADMIN','position':'Chuyên viên Pick Pack 1291','email':'visual@example.invalid'}),encoding='utf-8')
    master={'ok':True,'master_revision':9,'staff':[{'mnv':'30011','full_name':'Nguyễn Văn A','main_position':'Pick','supplier':'NLV','department':'Pick Pack','site':'1291','warehouse':'HY1'},{'mnv':'30050','full_name':'Trần Thị B','main_position':'Pick','supplier':'NTH','department':'Pick Pack','site':'1291','warehouse':'HY1'}],'pdas':[{'serial':'NLS-MT90-0012345','last5':'12345','status':'Nguyên vẹn'},{'serial':'NLS-MT90-0067890','last5':'67890','status':'Màn hình xước nhẹ'},{'serial':'NLS-MT90-0099999','last5':'99999','status':'Nguyên vẹn'}],'user_picks':['HY1.OUT.01','HY1.OUT.02'],'pack_bundles':[{'table':'D1','user_pack':'PACK01'},{'table':'D2','user_pack':'PACK02'},{'table':'D3','user_pack':'PACK03'}]}
    ...
    record('fixture.txt',f'date={date} uid={uid} gid={gid} holders=2 master_pdas=3 pack_tables=D1,D2,D3')
...
def open_exchange(d,wh):
    adb('shell','am','force-stop',PKG,check=False);r=adb('shell','am','start','-W','-n',f'{PKG}/{ACT}','--es','module','PDA_EXCHANGE','--es','login','0987654321','--es','name','Nguyễn Văn Tâm','--es','role','ADMIN','--es','position','Chuyên viên Pick Pack 1291','--es','email','visual@example.invalid',check=False);record(f'{d}-start.txt',r.stdout);assert 'Error:' not in r.stdout;time.sleep(2.5);gate(f'{d}-exchange');shot(d,'10-pda-exchange',wh)

def main():
    if '--preflight' in os.sys.argv:preflight();return
    OUT.mkdir(parents=True,exist_ok=True);verify_candidate(os.environ['APK']);adb('wait-for-device');seed_fixture();rows=[]
    try:
        for size,density,wh in MATRIX:
            d=f'{size}x{density}';(OUT/d).mkdir(parents=True,exist_ok=True);adb('shell','wm','size',size);adb('shell','wm','density',density);time.sleep(.8);open_exchange(d,wh);rows.append(f'{d}: pda_exchange=PASS route_window=PASS png=PASS human_markers=REQUIRED')
    except Exception as e:
        record('failure.txt',repr(e));record('logcat-tail.txt',adb('logcat','-d','-t','250',check=False).stdout);raise
