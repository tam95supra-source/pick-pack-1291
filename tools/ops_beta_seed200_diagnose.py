...
ROOT=pathlib.Path(__file__).resolve().parents[1]; OUT=pathlib.Path('/tmp/beta-seed200-receipt.json'); TODAY=datetime.now(ZoneInfo('Asia/Bangkok')).strftime('%Y-%m-%d'); D1=''
...
try:
 ...
 event_entities={str(x.get('entity_id') or '') for x in events}; attendance_ids={str(x.get('session_id') or '') for x in attendance}; seen=[]
 for x in events:
  m=re.match(r'^OWNER_SEED200_20260904_(\d{3})_',str(x.get('event_id') or ''))
  if m: seen.append(int(m.group(1)))
 mx=max(seen) if seen else 0; missing=[i for i in range(1,mx+1) if i not in set(seen)]
 leases=d1("SELECT resource_type,resource_id,mnv,session_id,acquired_event_id FROM resource_leases WHERE business_date="+q(TODAY)+" ORDER BY resource_type,resource_id")
 packmap=d1("SELECT shift,pack_table,user_pack,available FROM resource_pack_map WHERE available=1 ORDER BY shift,pack_table")
 consumption=d1("SELECT resource_type,resource_id,mnv,first_event_id FROM resource_daily_consumption WHERE business_date="+q(TODAY)+" ORDER BY resource_type,resource_id")
 result={'project':'APK PICK PACK 1291','channel':'BETA','operation':'OWNER_SEED_200_PARTIAL_DIAGNOSE','business_date':TODAY,'status':'PASS','authority':a,'counts':counts,'owner_seed_event_count':len(events),'attendance_count':len(attendance),'attendance_all_backed_by_owner_seed_event_entity':not(attendance_ids-event_entities),'max_seed_index_seen':mx,'missing_indices_through_max':missing,'resource_leases':leases,'resource_pack_map':packmap,'resource_daily_consumption':consumption,'stable_write_attempts':0}
 OUT.write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print(json.dumps({'status':'PASS','counts':counts,'owner_seed_event_count':len(events),'max_seed_index_seen':mx,'missing_indices_through_max':missing,'resource_leases':leases,'resource_pack_map':packmap,'resource_daily_consumption':consumption},ensure_ascii=False))
