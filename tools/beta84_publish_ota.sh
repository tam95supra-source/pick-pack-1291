...
APK=/tmp/beta84-candidate/pick-pack-1291-public-beta-$VERSION.apk
META=/tmp/beta84-candidate/release-meta.json
VERIFY=/tmp/beta84-verify/receipt.json
...
jq -nc --arg d "Beta84 exact OTA; SHA256 $SHA; candidate run $(jq -r '.candidate_run_id' "$R"); artifact $(jq -r '.candidate_artifact_id' "$R"); signer $SIGNER" '{description:$d}' > "$E/meta.json"
