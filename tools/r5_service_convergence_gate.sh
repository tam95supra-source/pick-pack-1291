...
r5_service_convergence_gate(){
  ...
  r5_status(){
    local kind=$1 target=$2 http
    if [[ "$kind" == PDA ]]; then
      http=$(curl -sS --connect-timeout 10 --max-time 20 -o "$target" -w '%{http_code}' \
        -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
        --data-binary '{"action":"sync_status"}' "$SERVICE_URL/v1/legacy-sync" || printf 000)
      [[ "$http" =~ ^2 ]] || { echo "R5_PDA_STATUS_HTTP_$http" >&2; cat "$target" >&2 || true; return 31; }
      jq -e '.ok==true and .mode=="APP_SERVICE_D1" and (.business_date|type=="string") and (.day_revisions|type=="object")' "$target" >/dev/null
      jq -r '.service_telemetry.db_rows_read // 0' "$target" >> "$out/status-rows.txt"
    else
      http=$(curl -sS --connect-timeout 10 --max-time 20 -o "$target" -w '%{http_code}' \
        -H "Authorization: Bearer $TOKEN" "$SERVICE_URL/v1/sync/status" || printf 000)
      [[ "$http" =~ ^2 ]] || { echo "R5_WEB_STATUS_HTTP_$http" >&2; cat "$target" >&2 || true; return 32; }
      jq -e '.ok==true and .contract=="LOCAL_FIRST_REVISION_V1" and (.business_window|type=="array") and (.business_window|length)>0' "$target" >/dev/null
  ...
  r5_client_delta(){
    ...
    while true; do
      ...
      if [[ "$kind" == PDA ]]; then
        ...
        http=$(curl -sS --connect-timeout 10 --max-time 20 -o "$resp" -w '%{http_code}' \
          -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' \
          --data-binary "$body" "$SERVICE_URL/v1/legacy-sync" || printf 000)
      else
        http=$(curl -sS --connect-timeout 10 --max-time 20 -o "$resp" -w '%{http_code}' \
          -H "Authorization: Bearer $TOKEN" \
          "$SERVICE_URL/v1/delta/day?business_date=$date&after_revision=$cursor&limit=250&client_source=WEB" || printf 000)
      fi
      [[ "$http" =~ ^2 ]] || { echo "R5_DELTA_HTTP_$http trial=$trial client=$client kind=$kind" >&2; cat "$resp" >&2 || true; return 44; }
      jq -e '.ok==true and (.reset_required != true)' "$resp" >/dev/null
  ...
  local trial pda_status web_status date web_date pda_cursor web_cursor eid t0 ack i kind cursor
  for trial in $(seq 1 "$trials"); do
    pda_status="$out/status-pda-$trial.json"
    web_status="$out/status-web-$trial.json"
    r5_status PDA "$pda_status"
    r5_status WEB "$web_status"
    date=$(jq -r '.business_date' "$pda_status")
    web_date=$(jq -r '.business_window[0].business_date' "$web_status")
    [[ "$date" == "$web_date" && "$date" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]]
    pda_cursor=$(jq -r --arg d "$date" '.day_revisions[$d] // -1' "$pda_status")
    web_cursor=$(jq -r --arg d "$date" '[.business_window[]|select(.business_date==$d)|.revision][0] // -1' "$web_status")
    [[ "$pda_cursor" =~ ^[0-9]+$ && "$web_cursor" =~ ^[0-9]+$ && "$pda_cursor" == "$web_cursor" ]]
  ...
  [[ $(wc -l < "$out/status-rows.txt") -eq $((trials*2)) ]]
...
assert len(rows)==50
assert sum(r['kind']=='PDA' for r in rows)==30
assert sum(r['kind']=='WEB' for r in rows)==20
...
receipt={
  'status':'PASS','classification':'EXACT_DEPLOYED_SERVICE_REAL_TRANSPORT_5_LOGICAL_CLIENT_FANOUT',
  'clients':{'total':5,'android_pda':3,'web':2},'trials':10,'samples':50,
  'transport_paths':{'android_pda_status':'POST /v1/legacy-sync sync_status','android_pda_delta':'POST /v1/legacy-sync sync_delta','web_status':'GET /v1/sync/status','web_delta':'GET /v1/delta/day'},
  ...
  'hot_path_d1_rows_read':{
    ...
    'source':'response.service_telemetry on exact deployed R5 service using real PDA/Web transport endpoints'
  },
  'normalized_max_day':{
