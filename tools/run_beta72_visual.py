...
# ACTIVE Pick fixture intentionally has no USER_PICK assignment. This proves
# the UI fallback requested by OWNER when hy1.outbound user is not used.
src = src.replace(
...
# Add a clean employee with no session for the VÀO CA Pick/Pack UI gate.
src = src.replace(
...
# Canonical master mapping: User Pack is selected only from the chosen Pack Table.
src = src.replace(
    "'pack_bundles':[]",
...
# The app refreshes its header every 750 ms, so Android 10 uiautomator can wait
# forever for an accessibility-idle window. Visual evidence therefore uses the
# already-proven raw screenshot/coordinate path; semantic owner rules are gated
# in source before build and are then checked by human inspection on real pixels.
