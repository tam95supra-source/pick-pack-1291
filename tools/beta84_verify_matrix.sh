...
META=/tmp/beta84-candidate/release-meta.json;APK=/tmp/beta84-candidate/pick-pack-1291-public-beta-$VERSION.apk
...
OUT=/tmp/beta84-verify;rm -rf "$OUT";mkdir -p "$OUT"
...
for spec in '320 568 160' '360 640 180' '480 800 240'; do
  ...
  set +e
  timeout 150s adb shell am instrument -w -r -e mode "$MODE" -e tag "$TAG" -e mnv 981820081 -e mnv2 981820082 -e mnv3 981820083 vn.pickpack1291.verify/.Beta84UiChecksInstrumentation >"$OUT/$TAG-instrument.txt" 2>&1
  RC=$?
  set -e
  test "$RC" = 0;grep -Fq 'INSTRUMENTATION_CODE: 0' "$OUT/$TAG-instrument.txt"
  mkdir -p "$OUT/$TAG"
  adb pull "/sdcard/Android/data/$PKG/files/beta84-visual/." "$OUT/$TAG/" >/dev/null
  if [[ "$MODE" == checks ]]; then
    adb shell cat "/data/user/0/$PKG/shared_prefs/pp_beta84_verify.xml" >"$OUT/$TAG-flags.xml"
