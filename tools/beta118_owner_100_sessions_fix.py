...
seed=Path('.github/workflows/beta118-owner-seed-100-sessions.yml')
w=seed.read_text(encoding='utf-8')
old_shift="shifts=['CA 1','CA HC','CA 2']"
new_shift="shifts=['Ca 1','Ca HC','Ca 2']"
if old_shift not in w:
    raise SystemExit('seed shift anchor missing')
w=w.replace(old_shift,new_shift,1)
seed.write_text(w,encoding='utf-8')
...
seed=Path('.github/workflows/beta118-owner-seed-100-sessions.yml').read_text(encoding='utf-8')
...
assert "shifts=['Ca 1','Ca HC','Ca 2']" in seed
assert "shifts=['CA 1','CA HC','CA 2']" not in seed
items=receipt['items']
