#!/usr/bin/env bash
set -Eeuo pipefail

: "${GH_TOKEN:?}"
: "${GOOGLE_OAUTH_CLIENT_ID:?}"
: "${GOOGLE_OAUTH_CLIENT_SECRET:?}"
: "${GOOGLE_OAUTH_REFRESH_TOKEN:?}"
: "${GAS_SCRIPT_ID:?}"
: "${GAS_DEPLOYMENT_ID:?}"

SOURCE_RUN_ID=32754196617
ARTIFACT_ID=9530301628
SOURCE_SHA=63917d672aeced142cacda925300978deef65277
TARGET_VERSION=0.4.2-beta.68
TARGET_CODE=74
TARGET_PACKAGE=vn.pickpack1291.app.beta.publicbeta
APK_NAME=pick-pack-1291-public-beta-0.4.2-beta.68.apk
EXPECTED_SHA=34554a19621be73ebc1e3bd64bef547959f5843f8d0fc902b5bcc8d573cf4641
EXPECTED_SIZE=13097861
EXPECTED_SIGNER_SHA256=d180450ae47ac6e8daf26840308e62bd602d5f8d6ac12ee0da58e5eb1a44731e
BETA_FOLDER_ID=1WMXI-8-Z1mbY2v11noYFHe_eoMNiNZXg
TAG=v0.4.2-beta.68-publicbeta
GAS_URL='https://script.google.com/macros/s/AKfycbzbEoGfbNg6s2HnP-gUpcBJ7mMIkVBtYuQKMndb9seDV2c55lQwSUO1GZ-LtQ2CxMCauA/exec'
OUT=/tmp/release-result
mkdir -p /tmp/candidate /tmp/evidence
: > "$OUT"

# A. Immutable visual/candidate lock + current live authority.
grep -qx 'verdict=PASS' ops/beta68-v4-visual-inspection.txt
grep -qx "source_sha=$SOURCE_SHA" ops/beta68-v4-visual-inspection.txt
grep -qx "candidate_run_id=$SOURCE_RUN_ID" ops/beta68-v4-visual-inspection.txt
grep -qx "candidate_artifact_id=$ARTIFACT_ID" ops/beta68-v4-visual-inspection.txt
grep -qx "apk_sha256=$EXPECTED_SHA" ops/beta68-v4-visual-inspection.txt
grep -qx "apk_size=$EXPECTED_SIZE" ops/beta68-v4-visual-inspection.txt
grep -qx 'matrix=320x568,360x640,480x800' ops/beta68-v4-visual-inspection.txt
grep -qx 'login_model=SIMPLE_APP_THEME_V1' ops/beta68-v4-visual-inspection.txt
grep -qx 'human_inspection=PASS' ops/beta68-v4-visual-inspection.txt
MAIN_BEFORE=$(curl -fsSL -H "Authorization: Bearer $GH_TOKEN" -H 'Accept: application/vnd.github+json' "https://api.github.com/repos/$GITHUB_REPOSITORY/branches/main" | jq -r '.commit.sha')
[[ "$MAIN_BEFORE" =~ ^[0-9a-f]{40}$ ]]
curl -fsSL -H 'content-type: application/json' "$GAS_URL" -d '{"action":"update_check","channel":"STABLE","current_version":"0.1.0-stable"}' > /tmp/stable-before.json
curl -fsSL -H 'content-type: application/json' "$GAS_URL" -d '{"action":"update_check","channel":"BETA","current_version":"0.4.2-beta.66"}' > /tmp/beta-before.json
jq -e '.ok==true' /tmp/stable-before.json >/dev/null
jq -e '.ok==true' /tmp/beta-before.json >/dev/null
OTA_SOURCE_BEFORE=$(jq -r '.source // empty' /tmp/beta-before.json)
[[ -n "$OTA_SOURCE_BEFORE" ]]
printf 'ota_source_before=%s\nmain_before=%s\nvisual_lock=PASS\n' "$OTA_SOURCE_BEFORE" "$MAIN_BEFORE" >> "$OUT"

# B. Exact locked candidate bytes.
curl -fsSL -H "Authorization: Bearer $GH_TOKEN" -H 'Accept: application/vnd.github+json' "https://api.github.com/repos/$GITHUB_REPOSITORY/actions/artifacts/$ARTIFACT_ID/zip" -o /tmp/candidate.zip
unzip -q /tmp/candidate.zip -d /tmp/candidate
APK=$(find /tmp/candidate -type f -name "$APK_NAME" -print -quit)
[[ -n "$APK" && -s "$APK" ]]
[[ "$(sha256sum "$APK"|awk '{print $1}')" == "$EXPECTED_SHA" ]]
[[ "$(stat -c '%s' "$APK")" == "$EXPECTED_SIZE" ]]
SUM=$(find /tmp/candidate -type f -name 'SHA256SUMS-*.txt' -print -quit)
[[ -n "$SUM" ]] && grep -qx "$EXPECTED_SHA  $APK_NAME" "$SUM"
META=$(find /tmp/candidate -type f -name release-meta.json -print -quit)
jq -e --arg v "$TARGET_VERSION" --argjson c "$TARGET_CODE" --arg p "$TARGET_PACKAGE" --arg s "$SOURCE_SHA" --arg h "$EXPECTED_SHA" --argjson z "$EXPECTED_SIZE" --arg signer "$EXPECTED_SIGNER_SHA256" '.version_name==$v and .version_code==$c and .package==$p and .source_sha==$s and .apk_sha256==$h and .apk_size==$z and .signer_sha256==$signer and .stable_publish=="FORBIDDEN" and .visual_model=="SIMPLE_APP_THEME_V1"' "$META" >/dev/null
echo 'artifact_exact_bytes=PASS' >> "$OUT"

# C. Apps Script auth and exact original GAS snapshot.
RESP=$(curl -fsS https://oauth2.googleapis.com/token -H 'Content-Type: application/x-www-form-urlencoded' \
  --data-urlencode "client_id=$GOOGLE_OAUTH_CLIENT_ID" \
  --data-urlencode "client_secret=$GOOGLE_OAUTH_CLIENT_SECRET" \
  --data-urlencode "refresh_token=$GOOGLE_OAUTH_REFRESH_TOKEN" \
  --data-urlencode 'grant_type=refresh_token')
ACCESS_TOKEN=$(jq -r '.access_token // empty' <<<"$RESP"); [[ -n "$ACCESS_TOKEN" ]]
SCRIPT_ID=$(printf '%s' "$GAS_SCRIPT_ID"|tr -d '\r\n\t ')
RAW=$(printf '%s' "$GAS_DEPLOYMENT_ID"|tr -d '\r\n\t '); DEPLOYMENT_ID="$RAW"; if [[ "$RAW" =~ /s/([^/]+)/ ]]; then DEPLOYMENT_ID="${BASH_REMATCH[1]}"; fi
echo "::add-mask::$ACCESS_TOKEN"; echo "::add-mask::$SCRIPT_ID"; echo "::add-mask::$DEPLOYMENT_ID"
curl -fsS -H "Authorization: Bearer $ACCESS_TOKEN" "https://script.googleapis.com/v1/projects/$SCRIPT_ID/content" > /tmp/live-original.json
curl -fsS -H "Authorization: Bearer $ACCESS_TOKEN" "https://script.googleapis.com/v1/projects/$SCRIPT_ID/deployments/$DEPLOYMENT_ID" > /tmp/deployment-before.json
python3 - <<'PY'
import json,hashlib
j=json.load(open('/tmp/live-original.json'))
assert any(f.get('name')=='PICK_PACK_API' and f.get('source') for f in j.get('files',[]))
canon='\n'.join(f.get('name','')+'\0'+f.get('type','')+'\0'+f.get('source','') for f in j.get('files',[]))
open('/tmp/original-canon.sha','w').write(hashlib.sha256(canon.encode()).hexdigest())
PY

# Obtain a short-lived signed URL to this exact candidate ZIP; GAS can fetch it without Drive OAuth listing permission.
HDR=$(mktemp)
curl -sS -D "$HDR" -o /dev/null -H "Authorization: Bearer $GH_TOKEN" -H 'Accept: application/vnd.github+json' "https://api.github.com/repos/$GITHUB_REPOSITORY/actions/artifacts/$ARTIFACT_ID/zip"
ARTIFACT_URL=$(awk 'BEGIN{IGNORECASE=1}/^location:/{sub(/\r$/,"");sub(/^location:[[:space:]]*/,"");print;exit}' "$HDR")
[[ "$ARTIFACT_URL" == https://* ]]
echo "::add-mask::$ARTIFACT_URL"

GAS_MUTATED=0
restore_gas(){
  if [[ "$GAS_MUTATED" != 1 ]]; then return 0; fi
  jq '{files:.files}' /tmp/live-original.json > /tmp/original-put.json
  H=$(curl -sS -o /tmp/restore-put.out -w '%{http_code}' -X PUT -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Content-Type: application/json' --data-binary @/tmp/original-put.json "https://script.googleapis.com/v1/projects/$SCRIPT_ID/content")
  [[ "$H" == 200 ]] || { cat /tmp/restore-put.out >&2; return 1; }
  H=$(curl -sS -o /tmp/restore-ver.json -w '%{http_code}' -X POST -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Content-Type: application/json' -d '{"description":"Restore exact pre-Beta68 source after temporary Drive helper"}' "https://script.googleapis.com/v1/projects/$SCRIPT_ID/versions")
  [[ "$H" == 200 ]] || { cat /tmp/restore-ver.json >&2; return 1; }
  V=$(jq -r '.versionNumber' /tmp/restore-ver.json); [[ "$V" =~ ^[0-9]+$ ]]
  jq -nc --arg sid "$SCRIPT_ID" --argjson v "$V" '{deploymentConfig:{scriptId:$sid,versionNumber:$v,manifestFileName:"appsscript",description:"Restore exact pre-Beta68 source"}}' > /tmp/restore-deploy.json
  H=$(curl -sS -o /tmp/restore-deploy.out -w '%{http_code}' -X PUT -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Content-Type: application/json' --data-binary @/tmp/restore-deploy.json "https://script.googleapis.com/v1/projects/$SCRIPT_ID/deployments/$DEPLOYMENT_ID")
  [[ "$H" == 200 ]] || { cat /tmp/restore-deploy.out >&2; return 1; }
  curl -fsS -H "Authorization: Bearer $ACCESS_TOKEN" "https://script.googleapis.com/v1/projects/$SCRIPT_ID/content" > /tmp/live-after-restore.json
  python3 - <<'PY'
import json,hashlib
j=json.load(open('/tmp/live-after-restore.json'))
canon='\n'.join(f.get('name','')+'\0'+f.get('type','')+'\0'+f.get('source','') for f in j.get('files',[]))
after=hashlib.sha256(canon.encode()).hexdigest(); before=open('/tmp/original-canon.sha').read().strip(); assert after==before,(before,after)
PY
  GAS_MUTATED=0
  echo 'gas_source_restored=PASS' >> "$OUT"
}
trap 'rc=$?; restore_gas || rc=99; exit $rc' EXIT

# D. Install a nonce-gated temporary helper. It only accepts the approved Beta folder and exact locked SHA/size.
NONCE=$(openssl rand -hex 32); echo "::add-mask::$NONCE"
python3 - "$NONCE" <<'PY'
import json,sys
nonce=sys.argv[1]
j=json.load(open('/tmp/live-original.json'))
f=next(x for x in j['files'] if x.get('name')=='PICK_PACK_API')
s=f['source']
anchor="    if (action === 'health') return ppJson_(ppHealth_());"
if anchor not in s: raise SystemExit('HEALTH_ACTION_ANCHOR_MISSING')
route="    if (action === '__b68_exact_drive_backup') return ppJson_(ppB68ExactDriveBackupTmp_(body));"
s=s.replace(anchor,anchor+'\n'+route,1)
helper='''
function ppB68ExactDriveBackupTmp_(body){
  if(String(body.token||'')!=='__NONCE__')return {ok:false,error:'FORBIDDEN'};
  if(String(body.folder_id||'')!=='1WMXI-8-Z1mbY2v11noYFHe_eoMNiNZXg')return {ok:false,error:'TARGET_MISMATCH'};
  const expected=String(body.sha||'').toLowerCase(), expectedSize=Number(body.size||0), apkName='pick-pack-1291-public-beta-0.4.2-beta.68.apk', sumName='SHA256SUMS-0.4.2-beta.68.txt';
  const r=UrlFetchApp.fetch(String(body.artifact_url||''),{muteHttpExceptions:true,followRedirects:true}); if(r.getResponseCode()!==200)return {ok:false,error:'FETCH_'+r.getResponseCode()};
  const blobs=Utilities.unzip(r.getBlob()); let apk=null; blobs.forEach(function(b){if(String(b.getName()||'')===apkName)apk=b;}); if(!apk)return {ok:false,error:'APK_MISSING'};
  const bytes=apk.getBytes(); if(bytes.length!==expectedSize)return {ok:false,error:'SIZE_MISMATCH',size:bytes.length};
  const dig=Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256,bytes), sha=dig.map(function(b){return ('0'+((b+256)%256).toString(16)).slice(-2);}).join(''); if(sha!==expected)return {ok:false,error:'SHA_MISMATCH',sha256:sha};
  const folder=DriveApp.getFolderById(String(body.folder_id)); let af=null,it=folder.getFilesByName(apkName);
  while(it.hasNext()){let x=it.next();if(x.getSize()===expectedSize){const xb=x.getBlob().getBytes(),xd=Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256,xb),xh=xd.map(function(b){return ('0'+((b+256)%256).toString(16)).slice(-2);}).join('');if(xh===expected){af=x;break;}}}
  if(!af)af=folder.createFile(apk.copyBlob().setName(apkName));
  af.setDescription('Beta68 SIMPLE_APP_THEME_V1 | SHA256 '+expected+' | exact candidate run 32754196617');
  let sf=null,si=folder.getFilesByName(sumName); while(si.hasNext()){let x=si.next();if(String(x.getBlob().getDataAsString()).indexOf(expected)>=0){sf=x;break;}} if(!sf)sf=folder.createFile(sumName,expected+'  '+apkName+'\\n','text/plain');
  try{af.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);}catch(_){} try{sf.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);}catch(_){}
  return {ok:true,apk_file_id:af.getId(),sum_file_id:sf.getId(),sha256:sha,size:af.getSize(),apk_name:af.getName(),sum_name:sf.getName()};
}
'''.replace('__NONCE__',nonce)
f['source']=s+helper
open('/tmp/helper-content.json','w').write(json.dumps({'files':j['files']},ensure_ascii=False))
PY
H=$(curl -sS -o /tmp/helper-put.out -w '%{http_code}' -X PUT -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Content-Type: application/json' --data-binary @/tmp/helper-content.json "https://script.googleapis.com/v1/projects/$SCRIPT_ID/content")
[[ "$H" == 200 ]] || { cat /tmp/helper-put.out; exit 1; }
GAS_MUTATED=1
H=$(curl -sS -o /tmp/helper-ver.json -w '%{http_code}' -X POST -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Content-Type: application/json' -d '{"description":"TEMP Beta68 exact Drive backup helper"}' "https://script.googleapis.com/v1/projects/$SCRIPT_ID/versions")
[[ "$H" == 200 ]] || { cat /tmp/helper-ver.json; exit 1; }
V=$(jq -r '.versionNumber' /tmp/helper-ver.json); [[ "$V" =~ ^[0-9]+$ ]]
jq -nc --arg sid "$SCRIPT_ID" --argjson v "$V" '{deploymentConfig:{scriptId:$sid,versionNumber:$v,manifestFileName:"appsscript",description:"TEMP Beta68 exact Drive backup helper"}}' > /tmp/helper-deploy.json
H=$(curl -sS -o /tmp/helper-deploy.out -w '%{http_code}' -X PUT -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Content-Type: application/json' --data-binary @/tmp/helper-deploy.json "https://script.googleapis.com/v1/projects/$SCRIPT_ID/deployments/$DEPLOYMENT_ID")
[[ "$H" == 200 ]] || { cat /tmp/helper-deploy.out; exit 1; }

# E. Transport exact bytes into approved Drive folder.
jq -nc --arg token "$NONCE" --arg folder "$BETA_FOLDER_ID" --arg url "$ARTIFACT_URL" --arg sha "$EXPECTED_SHA" --argjson size "$EXPECTED_SIZE" '{action:"__b68_exact_drive_backup",token:$token,folder_id:$folder,artifact_url:$url,sha:$sha,size:$size}' > /tmp/transport-request.json
OK=false
for i in $(seq 1 25); do
  curl -fsSL --retry 2 -H 'content-type: application/json' "$GAS_URL" --data-binary @/tmp/transport-request.json > /tmp/transport.json || true
  if jq -e --arg h "$EXPECTED_SHA" --argjson z "$EXPECTED_SIZE" --arg n "$APK_NAME" '.ok==true and .sha256==$h and .size==$z and .apk_name==$n' /tmp/transport.json >/dev/null 2>&1; then OK=true; break; fi
  sleep 4
done
[[ "$OK" == true ]] || { cat /tmp/transport.json; exit 1; }
DRIVE_APK_ID=$(jq -r '.apk_file_id // empty' /tmp/transport.json); DRIVE_SUM_ID=$(jq -r '.sum_file_id // empty' /tmp/transport.json)
[[ -n "$DRIVE_APK_ID" && -n "$DRIVE_SUM_ID" ]]
printf 'drive_file_id=%s\ndrive_sum_file_id=%s\ndrive_exact_bytes=PASS\n' "$DRIVE_APK_ID" "$DRIVE_SUM_ID" >> "$OUT"

# F. Restore GAS before OTA publication; exact source hash must match pre-helper snapshot.
restore_gas

# G. Publish exact same bytes as GitHub prerelease (current OTA authority may be GitHub Release).
cat > /tmp/release-notes.md <<'EOF'
## Beta68
- Màn đăng nhập đơn giản, chuyên nghiệp, đồng bộ theme ứng dụng; không dùng nền demo và không tự bật bàn phím khi mở.
- Đổi / Trả PDA chỉ hiển thị PDA đang được sử dụng.
- Hiển thị đúng Dữ liệu người dùng và Bộ nhớ đệm của ứng dụng.
- Bật lại tự động kiểm tra OTA khi mở hoặc quay lại ứng dụng; không polling nền.
- Bổ sung trạng thái chi tiết, chính xác cho Mạng, Đồng bộ và Dịch vụ.

Exact signed candidate from run 32754196617. Stable is not changed.
EOF
printf '%s  %s\n' "$EXPECTED_SHA" "$APK_NAME" > /tmp/SHA256SUMS-0.4.2-beta.68.txt
git fetch --tags origin
if gh release view "$TAG" >/dev/null 2>&1; then
  TAG_SHA=$(git rev-list -n1 "$TAG" 2>/dev/null || true)
  [[ "$TAG_SHA" == "$SOURCE_SHA" ]] || { echo "EXISTING_BETA68_TAG_WRONG_SOURCE:$TAG_SHA"; exit 20; }
  gh release edit "$TAG" --prerelease --title "Pick Pack 1291 $TARGET_VERSION" --notes-file /tmp/release-notes.md
else
  gh release create "$TAG" --target "$SOURCE_SHA" --prerelease --title "Pick Pack 1291 $TARGET_VERSION" --notes-file /tmp/release-notes.md
fi
gh release upload "$TAG" "$APK" /tmp/SHA256SUMS-0.4.2-beta.68.txt --clobber
rm -rf /tmp/github-live && mkdir -p /tmp/github-live
gh release download "$TAG" -p "$APK_NAME" -D /tmp/github-live
[[ "$(sha256sum /tmp/github-live/$APK_NAME|awk '{print $1}')" == "$EXPECTED_SHA" ]]
[[ "$(stat -c '%s' /tmp/github-live/$APK_NAME)" == "$EXPECTED_SIZE" ]]
printf 'github_prerelease=PASS\ngithub_tag=%s\n' "$TAG" >> "$OUT"

# H. Final public/live readback.
PASS=false
for n in $(seq 1 25); do
  curl -fsSL -H 'content-type: application/json' "$GAS_URL" -d '{"action":"update_check","channel":"BETA","current_version":"0.4.2-beta.66"}' > /tmp/ota-old.json
  if jq -e --arg v "$TARGET_VERSION" --argjson c "$TARGET_CODE" --arg h "$EXPECTED_SHA" --argjson z "$EXPECTED_SIZE" '.ok==true and .available==true and .version_name==$v and .version_code==$c and .sha256==$h and .size==$z' /tmp/ota-old.json >/dev/null 2>&1; then PASS=true; break; fi
  sleep 4
done
[[ "$PASS" == true ]] || { cat /tmp/ota-old.json; exit 1; }
OTA_SOURCE_AFTER=$(jq -r '.source // empty' /tmp/ota-old.json)
[[ "$OTA_SOURCE_AFTER" == "$OTA_SOURCE_BEFORE" ]] || { echo "OTA_AUTHORITY_CHANGED:$OTA_SOURCE_BEFORE->$OTA_SOURCE_AFTER"; exit 21; }
curl -fsSL -H 'content-type: application/json' "$GAS_URL" -d '{"action":"update_check","channel":"BETA","current_version":"0.4.2-beta.68"}' > /tmp/ota-current.json
jq -e --arg v "$TARGET_VERSION" --argjson c "$TARGET_CODE" '.ok==true and .available==false and .version_name==$v and .version_code==$c' /tmp/ota-current.json >/dev/null
LIVE_URL=$(jq -r '.apk_url // .download_url // .url // empty' /tmp/ota-old.json); [[ "$LIVE_URL" == https://* ]]
curl -fsSL -L --connect-timeout 15 --max-time 120 "$LIVE_URL" -o /tmp/ota-public.apk
[[ "$(sha256sum /tmp/ota-public.apk|awk '{print $1}')" == "$EXPECTED_SHA" ]]
[[ "$(stat -c '%s' /tmp/ota-public.apk)" == "$EXPECTED_SIZE" ]]
curl -fsSL -L --connect-timeout 15 --max-time 120 "https://drive.usercontent.google.com/download?id=$DRIVE_APK_ID&export=download&confirm=t" -o /tmp/drive-public.apk
[[ "$(sha256sum /tmp/drive-public.apk|awk '{print $1}')" == "$EXPECTED_SHA" ]]
[[ "$(stat -c '%s' /tmp/drive-public.apk)" == "$EXPECTED_SIZE" ]]
curl -fsSL -H 'content-type: application/json' "$GAS_URL" -d '{"action":"update_check","channel":"STABLE","current_version":"0.1.0-stable"}' > /tmp/stable-after.json
python3 - <<'PY'
import json
a=json.load(open('/tmp/stable-before.json')); b=json.load(open('/tmp/stable-after.json'))
keys=('source','channel','version_name','version_code','sha256','size','apk_url','available','reason')
assert {k:a.get(k) for k in keys}=={k:b.get(k) for k in keys},(a,b)
PY
MAIN_AFTER=$(curl -fsSL -H "Authorization: Bearer $GH_TOKEN" -H 'Accept: application/vnd.github+json' "https://api.github.com/repos/$GITHUB_REPOSITORY/branches/main"|jq -r '.commit.sha')
[[ "$MAIN_AFTER" == "$MAIN_BEFORE" ]]
printf 'ota_live=PASS\nota_source=%s\nota_version=%s\nota_code=%s\nota_sha256=%s\nota_size=%s\nota_url=%s\ndrive_public_bytes=PASS\nstable_unchanged=PASS\nmain_unchanged=PASS\n' "$OTA_SOURCE_AFTER" "$TARGET_VERSION" "$TARGET_CODE" "$EXPECTED_SHA" "$EXPECTED_SIZE" "$LIVE_URL" >> "$OUT"

# I. Immutable receipt.
{
  echo 'verdict=PASS'
  echo "release_run_id=$GITHUB_RUN_ID"
  echo "released_at=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  echo "source_sha=$SOURCE_SHA"
  echo "candidate_run_id=$SOURCE_RUN_ID"
  echo "candidate_artifact_id=$ARTIFACT_ID"
  echo "beta_name=$TARGET_VERSION"
  echo "beta_code=$TARGET_CODE"
  echo "package=$TARGET_PACKAGE"
  echo "apk_file=$APK_NAME"
  echo "apk_sha256=$EXPECTED_SHA"
  echo "apk_size=$EXPECTED_SIZE"
  echo "signer_sha256=$EXPECTED_SIGNER_SHA256"
  echo 'login_model=SIMPLE_APP_THEME_V1'
  echo "beta_folder_id=$BETA_FOLDER_ID"
  cat "$OUT"
  echo 'stable_name=0.1.0-stable'
  echo 'stable_code=1'
  echo 'stable_publish=FORBIDDEN'
} > ops/beta68-v4-release-result.txt
git config user.name github-actions[bot]
git config user.email 41898282+github-actions[bot]@users.noreply.github.com
git add ops/beta68-v4-release-result.txt
git commit -m 'ops: record Beta68 v4 exact OTA PASS'
git pull --rebase origin release/beta68-exact-20260824
git push origin HEAD:release/beta68-exact-20260824
trap - EXIT
