...
APK=/tmp/beta82-candidate/pick-pack-1291-public-beta-$VERSION.apk
META=/tmp/beta82-candidate/release-meta.json
VERIFY=/tmp/beta82-verify/receipt.json
...
jq -e --arg v "$VERSION" --arg h "$SHA" --argjson z "$SIZE" '
  ...
  .screenshot_count==24 and (.visual_sizes|sort)==(["320x568","360x640","480x800"]|sort)
' "$VERIFY" >/dev/null
...
jq -nc --arg d "Beta82 exact OTA; SHA256 $SHA; candidate run $(jq -r '.candidate_run_id' "$R"); artifact $(jq -r '.candidate_artifact_id' "$R"); signer $SIGNER" '{description:$d}' > "$E/meta.json"
