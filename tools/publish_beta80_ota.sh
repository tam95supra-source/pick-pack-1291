...
E=/tmp/beta80-publish
mkdir -p "$E"
TARGET_VERSION=0.4.2-beta.80
TARGET_CODE=86
PACKAGE=vn.pickpack1291.app.beta.publicbeta
SOURCE_SHA=38d6f08aa63600f4ef09fd524428beccebc4bb6f
CANDIDATE_RUN=33028423645
CANDIDATE_ARTIFACT=9629377960
VISUAL_ARTIFACT=9629445113
EXPECTED_SHA=1210bf57ff3bb48a723aa40d2efc8ec922c5e632e4c1d9928bf4dbe843654a69
EXPECTED_SIZE=13196221
...
SUM_NAME=SHA256SUMS-0.4.2-beta.80.txt
APK=/tmp/beta80-candidate/$APK_NAME
...
jq -e --arg s "$SOURCE_SHA" --arg h "$EXPECTED_SHA" --argjson z "$EXPECTED_SIZE" --arg signer "$EXPECTED_SIGNER" \
  '.version_name=="0.4.2-beta.80" and .version_code==86 and .package=="vn.pickpack1291.app.beta.publicbeta" and .source_sha==$s and .build_run==33028423645 and .apk_sha256==$h and .apk_size==$z and .signer_sha256==$signer and .candidate_locked==true and .stable_publish=="FORBIDDEN" and .service_run==33028423645 and (.service_artifact==null or .service_artifact==9629315986) and .historical_result=="3/3_SERVICE_D1_EXACT" and .outbound_result=="CRUD_DUP_GSHEET_PASS" and .authority_change=="NONE"' "$META" >/dev/null
...
read_update_check(){
  ...
  for attempt in 0 1 2; do
    if curl -fsSL --connect-timeout 15 --max-time 30 -H 'content-type: application/json' "$GAS_URL" \
      -d "{\"action\":\"update_check\",\"channel\":\"$channel\",\"current_version\":\"$current\"}" > "$out"; then
      if jq -e --arg ch "$channel" '.ok==true and .channel==$ch and (.available==true or .available==false)' "$out" >/dev/null 2>&1; then return 0; fi
    fi
    echo "ota_readback_contract_retry channel=$channel attempt=$attempt" >&2
    [[ "$attempt" -lt 2 ]] || break
    sleep $((2 + attempt * 4))
  done
...
PY
  ...
  read_update_check BETA 0.4.2-beta.79 "$E/beta-after-raw.json"
  jq -e --arg h "$EXPECTED_SHA" --arg u "$APK_URL" --argjson z "$EXPECTED_SIZE" \
    '.ok==true and .channel=="BETA" and .available==true and .version_name=="0.4.2-beta.80" and .sha256==$h and .size==$z and .apk_url==$u' \
    "$E/beta-after-raw.json" >/dev/null
  # Normalize version_code only after exact public bytes equal the locked VC86 candidate.
  jq '. + {version_code:(.version_code//86),version_code_evidence:"EXACT_PUBLIC_BYTES_EQUAL_LOCKED_CANDIDATE_VC86"}' \
