...
def ensure_uid():
    ...
    if ':' not in probe:
        adb('shell','monkey','-p',PKG,'1',check=False); time.sleep(.8); adb('shell','am','force-stop',PKG,check=False)
        probe=adb('shell','stat','-c','%u:%g',f'/data/user/0/{PKG}',check=False).stdout.strip()
    assert ':' in probe,probe
    return probe.split(':',1)
def seed_auth():
    uid,gid=ensure_uid(); OUT.mkdir(parents=True,exist_ok=True)
    p=OUT/'pick_pack_auth_session_v2.xml'
...
def dump_xml(name):
    adb('shell','uiautomator','dump','/sdcard/window.xml',check=False,timeout=12)
    raw=adb('shell','cat','/sdcard/window.xml',check=False,timeout=8).stdout
    rec(name,raw); return raw
...
def tap_node(raw,text,clazz=None,last=False):
    nodes=find_nodes(raw,text,clazz)
    assert nodes,f'node not found: {text} {clazz}'
    n,b=nodes[-1 if last else 0]; x=(b[0]+b[2])//2; y=(b[1]+b[3])//2; adb('shell','input','tap',str(x),str(y)); return b
def screenshot(name,wh):
    data=adb('exec-out','screencap','-p',text=False).stdout
    rec(name,data)
    # IHDR width/height offsets 16/20.
...
def launch_business():
    ...
    r=adb('shell','am','start','-W','-n',f'{PKG}/{ACT}','--es','module','BUSINESS','--es','login','tamnv2','--es','name','Nguyễn Văn Tâm','--es','role','SUPERADMIN','--es','position','superadmin','--es','email','tam95.supra@gmail.com',check=False,timeout=20)
    assert 'Error:' not in r.stdout,r.stdout; time.sleep(1.0)
def open_drop():
    raw=dump_xml('business.xml'); tap_node(raw,'Nhận hàng Rớt'); time.sleep(.7)
    raw=dump_xml('drop.xml'); assert 'NHẬN HÀNG RỚT' in raw or 'Nhận hàng Rớt' in raw
    for label in ('Vị trí','Tạo','Sửa','Xóa','Scan QR','DO','Số kiện','Thêm thông tin','Xóa toàn bộ'):
        assert label in raw,label
    return raw
def visible(raw,text,wh):
    nodes=find_nodes(raw,text)
    for _,b in nodes:
        if b[2]>0 and b[0]<wh[0] and b[3]>0 and b[1]<wh[1] and b[2]>b[0] and b[3]>b[1]: return True
    return False

def one_size(w,h):
    wh=(w,h); tag=f'{w}x{h}'
    adb('shell','wm','size',f'{w}x{h}'); adb('shell','wm','density','240'); time.sleep(.5)
    launch_business(); raw=open_drop(); screenshot(f'{tag}-top.png',wh); rec(f'{tag}-top.xml',raw)
    for label in ('Vị trí','Tạo','Sửa','Xóa','Scan QR','DO','Số kiện'):
        assert visible(raw,label,wh),f'{tag} top missing {label}'
    # Keyboard fixture: tap the Scan QR EditText/hint and capture real IME behavior.
    edits=find_nodes(raw,'Scan QR','android.widget.EditText')
    if edits:
        _,b=edits[0]; adb('shell','input','tap',str((b[0]+b[2])//2),str((b[1]+b[3])//2)); time.sleep(.7)
        screenshot(f'{tag}-keyboard.png',wh); rec(f'{tag}-keyboard.xml',dump_xml(f'{tag}-keyboard-dump.xml'))
        adb('shell','input','keyevent','4'); time.sleep(.4)
    # Scroll form to bottom to inspect primary/destructive actions on short PDA screens.
    for _ in range(3): adb('shell','input','swipe',str(w//2),str(int(h*.72)),str(w//2),str(int(h*.34)),'320'); time.sleep(.25)
    bottom=dump_xml(f'{tag}-bottom.xml'); screenshot(f'{tag}-bottom.png',wh)
    assert visible(bottom,'Thêm thông tin',wh),f'{tag} bottom missing add'
    assert visible(bottom,'Xóa toàn bộ',wh),f'{tag} bottom missing clear'
    act=adb('shell','dumpsys','activity','activities',check=False).stdout
    assert PKG in act and 'OperationsActivity' in act,f'{tag} wrong activity'
    rec(f'{tag}-activity.txt',act[-12000:])
