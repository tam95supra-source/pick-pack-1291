...
def main():
    ...
    shot('business.png')
    # At mdpi the Nhận hàng Rớt card is already fully visible in row two.
    # Open it before any scroll so a stale 240-dpi coordinate cannot route to
    # Công nhật or another neighbouring card.
    ...
    if not ime_visible():
        adb('shell', 'input', 'tap', '160', '198')
        time.sleep(1.2)
    assert ime_visible(), 'Scan QR focus did not open IME'
    shot('keyboard.png')
