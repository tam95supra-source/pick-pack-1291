...
def main():
 verify();adb('wait-for-device');adb('shell','svc','wifi','disable',check=False);adb('shell','svc','data','disable',check=False);adb('shell','wm','size','320x568');adb('shell','wm','density','240');time.sleep(.5);launch();shot('business.png');adb('shell','input','swipe','160','340','160','205','320');time.sleep(.5);shot('business-scrolled.png');adb('shell','input','tap','80','285');time.sleep(.8);adb('shell','wm','density','180');time.sleep(5.8);shot('drop-top.png');adb('shell','input','tap','160','215');time.sleep(.8);shot('keyboard.png');adb('shell','input','keyevent','4');time.sleep(.5)
 for _ in range(4):adb('shell','input','swipe','160','390','160','165','320');time.sleep(.2)
 shot('drop-bottom.png');rec('probe.json','{"status":"PROBE_CAPTURED","size":"320x568","density":180,"candidate_run":32875201581,"artifact_id":9573716441,"requires_human_inspection":true}\n');print('BETA76_VISUAL_PROBE_CAPTURED')
