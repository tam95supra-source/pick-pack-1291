...
E=/tmp/beta79-publish
mkdir -p "$E"
TARGET_VERSION=0.4.2-beta.79
TARGET_CODE=85
PACKAGE=vn.pickpack1291.app.beta.publicbeta
SOURCE_SHA=db96999844a31e7fed7d0f072fd0dd123fae1288
CANDIDATE_RUN=33020009122
CANDIDATE_ARTIFACT=9626192148
VISUAL_ARTIFACT=9626266511
EXPECTED_SHA=547e1242a7d0bb057332ce38c46313771da33235fc0e384a908c14207e26e056
EXPECTED_SIZE=13196165
EXPECTED_SIGNER=d180450ae47ac6e8daf26840308e62bd602d5f8d6ac12ee0da58e5eb1a44731e
PREV_VERSION=0.4.2-beta.78
PREV_CODE=84
PREV_SHA=73ebd3015f214f168af484433b3591b6ed85e784280e9a9f7e38a405291f2c6b
PREV_SIZE=13196165
BETA_FOLDER_ID=1WMXI-8-Z1mbY2v11noYFHe_eoMNiNZXg
APK_NAME=pick-pack-1291-public-beta-0.4.2-beta.79.apk
SUM_NAME=SHA256SUMS-0.4.2-beta.79.txt
APK=/tmp/beta79-candidate/$APK_NAME
META=/tmp/beta79-candidate/release-meta.json
...
jq -e --arg s "$SOURCE_SHA" --arg h "$EXPECTED_SHA" --argjson z "$EXPECTED_SIZE" --arg signer "$EXPECTED_SIGNER" \
  '.version_name=="0.4.2-beta.79" and .version_code==85 and .package=="vn.pickpack1291.app.beta.publicbeta" and .source_sha==$s and .build_run==33020009122 and .apk_sha256==$h and .apk_size==$z and .signer_sha256==$signer and .candidate_locked==true and .stable_publish=="FORBIDDEN" and .service_run==32977566159 and .service_artifact==9610145160 and .historical_result=="3/3_SERVICE_D1_EXACT" and .outbound_result=="CRUD_DUP_GSHEET_PASS" and .authority_change=="NONE"' "$META" >/dev/null
...
jq -e --arg h "$PREV_SHA" --arg th "$EXPECTED_SHA" \
  '(.version_name=="0.4.2-beta.78" and (.version_code//84)==84 and ((.sha256//"")==$h or .available==false)) or (.version_name=="0.4.2-beta.79" and (.version_code//85)==85 and ((.sha256//"")==$th or .available==false))' "$E/beta-before.json" >/dev/null
...
if jq -e --arg h "$EXPECTED_SHA" --argjson z "$EXPECTED_SIZE" \
    '.ok==true and .channel=="BETA" and .available==true and .version_name=="0.4.2-beta.79" and .sha256==$h and .size==$z and ((.apk_url//"")|length)>0' \
...
PY
  ...
  curl -fsSL --connect-timeout 15 --max-time 30 -H 'content-type: application/json' "$GAS_URL" \
    -d '{"action":"update_check","channel":"BETA","current_version":"0.4.2-beta.78"}' > "$E/beta-after-raw.json"
  jq -e --arg h "$EXPECTED_SHA" --arg u "$APK_URL" --argjson z "$EXPECTED_SIZE" \
    '.ok==true and .channel=="BETA" and .available==true and .version_name=="0.4.2-beta.79" and .sha256==$h and .size==$z and .apk_url==$u' \
    "$E/beta-after-raw.json" >/dev/null
  # Normalize version_code only after exact public bytes equal the locked VC85 candidate.
  jq '. + {version_code:(.version_code//85),version_code_evidence:"EXACT_PUBLIC_BYTES_EQUAL_LOCKED_CANDIDATE_VC85"}' \
...
s=f['source']
route="    if (action === 'update_check') return ppJson_(ppBeta79UpdateCheck547e1242_(body));"
helper=f'''\n// BETA79_EXACT_OTA_547E1242: exact locked candidate; Stable remains delegated to pre-existing ppUpdateCheck_.\nfunction ppBeta79UpdateCheck547e1242_(body) {{\n  const channel=ppFold_(body.channel||body._app_channel)==='STABLE'?'STABLE':'BETA';\n  if(channel==='STABLE') return ppUpdateCheck_(body);\n  const current=String(body.current_version||body._app_version||'').trim();\n  const version='0.4.2-beta.79', available=ppOtaCompare_(version,current)>0;\n  const out={{ok:true,source:'DRIVE_BETA',channel:'BETA',available:available,version_name:version,version_code:85,size:13196165,published_at:{json.dumps(published_at)},notes:'Phiên cũ mở đúng session_id vào màn QR để Thêm / Sửa / Xóa / Ra ca; Stable và authority không đổi.',mandatory:false}};\n  if(!available) return out;\n  out.sha256='547e1242a7d0bb057332ce38c46313771da33235fc0e384a908c14207e26e056';\n  out.apk_url={json.dumps(apk_url)};\n  return out;\n}}\n'''
lines=s.splitlines()
idx=[i for i,line in enumerate(lines) if "if (action === 'update_check')" in line]
assert len(idx)==1,f'update_check route count={len(idx)}'
lines[idx[0]]=route
s='\n'.join(lines)
if 'function ppBeta79UpdateCheck547e1242_(' not in s:
    s += helper
else:
    for needle in ('0.4.2-beta.79','version_code:85','547e1242a7d0bb057332ce38c46313771da33235fc0e384a908c14207e26e056',apk_url):
...
Path('/tmp/beta79-publish/gas-before.sha256').write_text(hashlib.sha256(canon(j.get('files',[])).encode()).hexdigest())
Path('/tmp/beta79-publish/gas-target.sha256').write_text(hashlib.sha256(canon(files).encode()).hexdigest())
PY
