...
method=r'''
  ...
  private JSONObject laborRow(String date,String id,String mnv,String name){return new JSONObject().put("labor_id",id).put("mnv",mnv).put("business_date",date).put("shift","Ca 1").put("labor_type","Công nhật").put("state","OPEN").put("start_at",date+"T01:00:00Z").put("end_at",JSONObject.NULL).put("note","").put("attendance_session_id","lf-session-"+mnv).put("full_name",name).put("supplier","LOCAL").put("position","Pick");}
  private JSONObject laborEvent(String id,String mnv){return new JSONObject().put("event_id","lf-event-"+id).put("event_type","LABOR_START").put("labor_id",id).put("entity_id",id).put("mnv",mnv).put("payload_json","{}");}
  ...
  private void finishActivity(final Activity a){runOnMainSync(new Runnable(){@Override public void run(){a.finish();}});SystemClock.sleep(350L);}
  ...
  private void runLocalFirst118Exact()throws Exception{
    ...
    saveLaborEvents(date,2);Activity business=open("BUSINESS");waitText("CẢNH BÁO: 2 CÔNG NHẬT CHƯA HOÀN THÀNH",true,false,3000L);
    saveLaborEvents(date,3);long tw=SystemClock.uptimeMillis();invokePrivateUiCallback(business,"laborWarningRealtimeRefresh");waitText("CẢNH BÁO: 3 CÔNG NHẬT CHƯA HOÀN THÀNH",true,false,3000L);long warningMs=SystemClock.uptimeMillis()-tw;require(warningMs<500L,"LOCAL_WARNING_RENDER_SLOW:"+warningMs);
    Bundle out=new Bundle();out.putString("localfirst_exact_ui","PASS");out.putString("two_rows_immediate","PASS");out.putString("stale_service_did_not_erase_two","PASS");out.putString("realtime_local_callback","PASS");out.putString("warning_local_callback","PASS");out.putString("two_rows_ms",String.valueOf(twoMs));out.putString("three_rows_ms",String.valueOf(threeMs));out.putString("warning_ms",String.valueOf(warningMs));finish(0,out);
  }
