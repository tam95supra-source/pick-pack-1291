...
VIS=ops/beta72-visual-inspection.json
E=/tmp/beta72-evidence
mkdir -p "$E" /tmp/beta72-candidate
...
PREV_SHA=5a8e29f5d50ac31010ebe2cd6e6096ffdd8bcd2b354007a7448878ae6eefec3b

test "$(jq -r '.stage' "$REQ")" = publish
test "$TARGET_VERSION" = 0.4.2-beta.72
test "$TARGET_CODE" = 78
test "$SOURCE_SHA" = 73d5432df09c33cda6554090838e5f55e41761ae
test "$SOURCE_RUN_ID" = 32808558173
test "$ARTIFACT_ID" = 9549037310
test "$EXPECTED_SHA" = fdeb006122f065591e82fe912a4a615c9c42a149568c2fc32f7d5b35db353caf
test "$EXPECTED_SIZE" = 13114245
...
jq -e --arg prev "$PREV_SHA" --arg next "$EXPECTED_SHA" '.ok==true and ((.version_name=="0.4.2-beta.71" and .sha256==$prev and .size==13114245) or (.version_name=="0.4.2-beta.72" and .sha256==$next and .size==13114245))' "$E/beta-before.json" >/dev/null
jq -e '.ok==true' "$E/stable-before.json" >/dev/null

curl -fsSL -H "Authorization: Bearer $GH_TOKEN" -H 'Accept: application/vnd.github+json' \
  "https://api.github.com/repos/$GITHUB_REPOSITORY/actions/artifacts/$ARTIFACT_ID/zip" -o /tmp/beta72-candidate.zip
unzip -q /tmp/beta72-candidate.zip -d /tmp/beta72-candidate
APK=$(find /tmp/beta72-candidate -type f -name "$APK_NAME" -print -quit)
META=$(find /tmp/beta72-candidate -type f -name release-meta.json -print -quit)
SUM=$(find /tmp/beta72-candidate -type f -name 'SHA256SUMS-*.txt' -print -quit)
...
restore_gas() {
  ...
  test "$(curl -sS -o /tmp/restore-ver.json -w '%{http_code}' -X POST -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Content-Type: application/json' -d '{"description":"Restore exact pre-Beta72 source"}' "https://script.googleapis.com/v1/projects/$SCRIPT_ID/versions")" = 200
  V=$(jq -r '.versionNumber' /tmp/restore-ver.json)
  jq -nc --arg sid "$SCRIPT_ID" --argjson v "$V" '{deploymentConfig:{scriptId:$sid,versionNumber:$v,manifestFileName:"appsscript",description:"Restore exact pre-Beta72 source"}}' > /tmp/restore-deploy.json
...
route="    if (action === '__beta72_exact_drive_upload') return ppJson_(ppBeta72ExactDriveUploadTmp_(body));"
notes="• User Pick / User Pack và nút Phát lại cùng dòng.\n• Pick bắt buộc PDA; Pack bắt buộc Bàn Pack + User Pack.\n• User Pack ràng buộc theo Bàn Pack.\n• Thời gian và vị trí trong ca hiển thị theo Tiêu đề: giá trị.\n• Không dùng User Pick hiển thị user cố định theo số điện thoại / họ tên."
helper=f'''
function ppBeta72ExactDriveUploadTmp_(body){{
  ...
  const expected={json.dumps(sha)},expectedSize={int(size)},apkName={json.dumps(name)},sumName='SHA256SUMS-0.4.2-beta.72.txt';
...
MUTATED=1
test "$(curl -sS -o /tmp/helper-ver.json -w '%{http_code}' -X POST -H "Authorization: Bearer $ACCESS_TOKEN" -H 'Content-Type: application/json' -d '{"description":"TEMP Beta72 exact Drive upload"}' "https://script.googleapis.com/v1/projects/$SCRIPT_ID/versions")" = 200
V=$(jq -r '.versionNumber' /tmp/helper-ver.json)
jq -nc --arg sid "$SCRIPT_ID" --argjson v "$V" '{deploymentConfig:{scriptId:$sid,versionNumber:$v,manifestFileName:"appsscript",description:"TEMP Beta72 exact Drive upload"}}' > /tmp/helper-deploy.json
...
jq -nc --arg token "$NONCE" --arg folder "$BETA_FOLDER_ID" --arg url "$ARTIFACT_URL" \
  '{action:"__beta72_exact_drive_upload",token:$token,folder_id:$folder,artifact_url:$url}' > /tmp/transport-request.json
OK=false
...
for i in 1 2 3 4 5 6 7 8; do
  curl -fsSL -H 'content-type: application/json' "$GAS_URL" -d '{"action":"update_check","channel":"BETA","current_version":"0.4.2-beta.71"}' > "$E/beta-after.json"
  if jq -e --arg h "$EXPECTED_SHA" --argjson z "$EXPECTED_SIZE" '.ok==true and .available==true and .version_name=="0.4.2-beta.72" and .sha256==$h and .size==$z and ((.apk_url // .download_url // .url // "")|startswith("https://"))' "$E/beta-after.json" >/dev/null 2>&1; then PASS=true; break; fi
  sleep $((i*3))
done
test "$PASS" = true
curl -fsSL -H 'content-type: application/json' "$GAS_URL" -d '{"action":"update_check","channel":"BETA","current_version":"0.4.2-beta.72"}' > "$E/beta-current.json"
jq -e --argjson z "$EXPECTED_SIZE" '.ok==true and .source=="GOOGLE_DRIVE" and .channel=="BETA" and .available==false and .version_name=="0.4.2-beta.72" and .size==$z' "$E/beta-current.json" >/dev/null
LIVE_URL=$(jq -r '.apk_url // .download_url // .url // empty' "$E/beta-after.json")
[[ "$LIVE_URL" == https://* ]]
curl -fsSL -L --connect-timeout 15 --max-time 180 "$LIVE_URL" -o /tmp/beta72-live.apk
test "$(sha256sum /tmp/beta72-live.apk | awk '{print $1}')" = "$EXPECTED_SHA"
test "$(stat -c '%s' /tmp/beta72-live.apk)" = "$EXPECTED_SIZE"
curl -fsSL -L --connect-timeout 15 --max-time 180 "https://drive.usercontent.google.com/download?id=$DRIVE_ID&export=download&confirm=t" -o /tmp/beta72-drive.apk
test "$(sha256sum /tmp/beta72-drive.apk | awk '{print $1}')" = "$EXPECTED_SHA"
test "$(stat -c '%s' /tmp/beta72-drive.apk)" = "$EXPECTED_SIZE"
...
python3 - <<'PY'
import json
a=json.load(open('/tmp/beta72-evidence/stable-before.json'));b=json.load(open('/tmp/beta72-evidence/stable-after.json'))
...
jq -nc --argjson run "$GITHUB_RUN_ID" --arg source "$SOURCE_SHA" --argjson candidate_run "$SOURCE_RUN_ID" --argjson artifact "$ARTIFACT_ID" \
  ...
  '{verdict:"PASS",release_run_id:$run,source_sha:$source,candidate_run_id:$candidate_run,candidate_artifact_id:$artifact,version_name:"0.4.2-beta.72",version_code:78,package:"vn.pickpack1291.app.beta.publicbeta",apk_sha256:$sha,apk_size:$size,signer_sha256:$signer,drive_file_id:$drive,ota_url:$url,ota_live:"PASS",superseded_live_version:"0.4.2-beta.71",superseded_live_sha256:"5a8e29f5d50ac31010ebe2cd6e6096ffdd8bcd2b354007a7448878ae6eefec3b",base_version:"0.4.2-beta.71",stable_unchanged:"PASS",main_unchanged:"PASS"}' \
  > ops/beta72-release-result.json
cp ops/beta72-release-result.json "$E/"
git config user.name github-actions[bot]
git config user.email 41898282+github-actions[bot]@users.noreply.github.com
git add ops/beta72-release-result.json
git commit -m 'ops: record Beta72 OTA PASS'
